...
import { Loader2, List, Repeat, Printer } from "lucide-react";
...
export default function BookingEditDialog({ booking, open, onOpenChange, onBookingChange }: BookingEditDialogProps) {
  ...
  const handlePrint = () => {
    ...
    const seriesBookings = booking.bookingGroupId
      ...
      : [];
    const isRecurringSeries = groupInfo && groupInfo.isRecurring && seriesBookings.length > 1;

    const roomName = rooms.find(r => r.id === booking.roomId)?.name || booking.roomName;
